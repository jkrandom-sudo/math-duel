...
def _problem(answer=4):
    return {"a": 2, "op": "+", "b": 2, "answer": answer}
...
class TestPlayRound(unittest.TestCase):
    def test_quit_returns_none(self):
        out = io.StringIO()
        with mock.patch.object(game.core, "make_problem", return_value=_problem()):
    ...
    def test_all_correct_win(self):
        ...
        with mock.patch.object(game.core, "make_problem", return_value=_problem()):
            result = game.play_round(_stub(), sound, StackedInput(["4"] * 8), out)
    ...
    def test_all_wrong_loss(self):
        ...
        sound = mock.MagicMock()
        with mock.patch.object(game.core, "make_problem", return_value=_problem()):
            result = game.play_round(_stub(), sound, StackedInput(["5"] * 8), out)
    ...
    def test_hint_halves_points(self):
        out = io.StringIO()
        with mock.patch.object(game.core, "make_problem", return_value=_problem()):
            result = game.play_round(_stub(), mock.MagicMock(), StackedInput(["hint", "4"] + ["skip"] * 7), out)
    ...
    def test_skip(self):
        out = io.StringIO()
        with mock.patch.object(game.core, "make_problem", return_value=_problem()):
            result = game.play_round(_stub(), mock.MagicMock(), StackedInput(["skip"] * 8), out)
    ...
    def test_invalid_answer_retries(self):
        out = io.StringIO()
        with mock.patch.object(game.core, "make_problem", return_value=_problem()):
            result = game.play_round(_stub(), mock.MagicMock(), StackedInput(["x", "4"] + ["skip"] * 7), out)
        self.assertEqual(result["correct"], 1)
        self.assertIn("Please enter", out.getvalue())
