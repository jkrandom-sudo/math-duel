    ...
    for round_no in range(1, total + 1):
        ...
        while True:
            ...
            if choice == "skip":
                streak = 0
                sound.incorrect()
                _print(out, i18n.t(lang, "skipped", answer=problem["answer"]))
                break
            if choice == "hint":
                parity, size = core.hint_for(problem)
                used_hint = True
                _print(out, i18n.t(lang, "hint", parity=i18n.t(lang, parity),
                                   size=i18n.t(lang, size)))
                continue
            parsed = core.parse_answer(raw)
            if parsed is None:
            ...
            if parsed == problem["answer"]:
                ...
                streak = 0
                sound.incorrect()
                _print(out, i18n.t(lang, "incorrect", answer=problem["answer"]))
            break
